...
我们仔细想下，实际上，可以根据第一步的走法把所有走法分为两类，第一类是第一步走了1个台阶，另一类是第一步走了2个台阶。所以n个台阶的走法就等于先走1阶后，n-1个台阶的走法，再加上先走2阶后，n-2个台阶的走法。用公式表示就是：
...
可以通过在代码中限制递归调用的最大深度的方式来解决这个问题。递归调用超过一定深度（比如1000）之后，我们就不继续往下再递归了，直接返回报错。还是电影院那个例子，我们可以改造成下面这样子，就可以避免堆栈溢出了。不过，我写的代码是伪代码，为了代码简洁，有些边界条件没有考虑，比如x<=0。
...
![Image text](https://github.com/QiuSYang/Data-Structure/blob/master/algorithm/recursive/images/2.png)
...
算法的思路就是：
    ...
    1. 一个集合要排列出所有情况，我们先确定排列结果中第一位的所有情况；
    2. 然后，再用相同的方法确定下一位，因此可以使用递归的方式来解决这个问题；
